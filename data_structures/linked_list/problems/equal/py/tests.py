...
import sys
import LinkedList
import equalSolution as solution
...
def TestMain(sol, log=sys.stdout, doNotLogPassed=True) -> bool:
    ...
    def TestPredefined(sol, log, doNotLogPassed=True) -> bool:
        class ArgsExpectedPairCollection:
            def __init__(self, count=60, minVal=-9, maxVal=9, minLen=0, maxLen=64):
                self.count  = count
                self.minVal = minVal
                self.maxVal = maxVal
                self.minLen = minLen
            ...
            def __iter__(self):
                ...
                for _ in range(self.count):
                    if random.choice([True, False]):
                        currLen  = random.randint(self.minVal, self.maxVal)
                        leftVals = rightVals = [random.randint(self.minVal, self.maxVal) for _ in range(currLen)]
                    else:
                        leftLen   = random.randint(self.minVal, self.maxVal)
                        rightLen  = random.randint(self.minVal, self.maxVal)

                        leftVals  = [random.randint(self.minVal, self.maxVal) for _ in range(leftLen)]
                        rightVals = [random.randint(self.minVal, self.maxVal) for _ in range(rightLen)]

                    expected  = leftVals == rightVals
                    leftHead  = LinkedList.ConvertArrayToLinkedList(leftVals)
                    rightHead = LinkedList.ConvertArrayToLinkedList(rightVals)

                    yield ((leftHead, rightHead, ), expected, )
