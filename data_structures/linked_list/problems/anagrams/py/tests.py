...
def TestMain(sol, log=sys.stdout, doNotLogPassed=True) -> bool:
    ...
    def TestPredefined(sol, log, doNotLogPassed=True) -> bool:
        class ArgsExpectedPairCollection:
            def GenerateChars(self, count: int) -> str:
                import random
                import string

                return [random.choice(string.ascii_lowercase) for _ in range(count)]

            def __init__(self, count=60, minLen=0, maxLen=16):
                self.count  = count
                self.minLen = minLen
                self.maxLen = maxLen
            ...
            def __iter__(self):
                ...
                for _ in range(self.count):
                    if random.choice([True, False]):
                        charCount  = random.randint(self.minLen, self.maxLen)
                        leftChars  = self.GenerateChars(charCount)

                        rightChars = leftChars[:]
                        random.shuffle(rightChars)
                    else:
                        leftCharCount = random.randint(self.minLen, self.maxLen)                        
                        leftChars     = self.GenerateChars(leftCharCount)

                        rightCharCount = random.randint(self.minLen, self.maxLen)
                        rightChars    = self.GenerateChars(rightCharCount)

                    expected  = sorted(leftChars) == sorted(rightChars)
                    leftHead  = LinkedList.ConvertArrayToLinkedList(leftChars)
                    rightHead = LinkedList.ConvertArrayToLinkedList(rightChars)                                               
